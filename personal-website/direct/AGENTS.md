...
- `scenarios.ts` defines stable light, dark, system-dark, long-content, and appearance-write-failure scenarios with exact fixture and direct coverage claims.
...
- The Bombadil campaign, five-scenario matrix, artifact verifier, and runner
  bind shared conservative exploration and exact trace attestation to every
  homepage world.
...
- Run `bun run fuzz:direct -- --time-limit 20s` after changing the homepage,
  ...
  derives all five campaigns from the exact Direct catalog; catalog drift must
  fail closed. Use `--campaign dark-wide` for a
...
- Inspect each exact local run below
  `artifacts/direct-bombadil/personal-monorepo-template-<campaign>/`. Preserve a
  useful failing `trace.jsonl`, its run manifest, and logs only in private
  diagnostic storage before retrying. Replay it
...
- Give every product-owned named snapshot an exact parser or type predicate
  that accepts `unknown` and rejects non-plain objects, accessors, symbols,
  proxies, unknown keys, and exceeded bounds.
...
- Scheduled CI preallocates one lowercase RFC 4122 UUID, passes Direct v0.7.9
  ...
  parsers. The upload step must remain `if: always()` and must never target the
  parent upload directory when UUID allocation did not complete.
...
- Keep the campaign descriptors on Direct's shared matrix, viewport, and
  ...
  Keep random action-dependent write-failure discovery run-lifetime eventual;
  the matrix policy separately requires its product Click and semantic change.
